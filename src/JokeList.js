...
class JokeList extends Component {
  ...
  /** Request jokes from API */
  async getJokes() {
    try {
      // Load jokes one at a time, adding not seen jokes.
      ...
      while (jokes.length < this.props.numJokesToGet) {
        ...
        if (!seenJokes.has(joke.id)) {
          ...
          joke.push({ ...joke, votes: jokeVotes[joke.id], locked: false});
        } else {
  ...
  vote (id, delta) {
    ...
    jokeVotes[id] = (jokeVotes[id] || 0) + data;
    window.localStorage.setItem("jokeVotes", JSON.stringify(jokeVotes));
    this.setState(st => ({
